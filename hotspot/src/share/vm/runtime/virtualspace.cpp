...
void ReservedSpace::initialize(size_t size, size_t alignment, bool large,
  ...
  if (base == NULL) {
    ...
    // 当对分配的堆地址的基址有要求时，调用os::attempt_reserve_memory_at()函数进行内存分配
    if (requested_address != 0) {
...
ReservedHeapSpace::ReservedHeapSpace(
    ...
    size_t alignment,       // 内存需要按alignment进行对齐
    bool large,             // 默认值为false， 表示不使用大页
    char* requested_address // 传递的值为NULL， 表示对分配的堆基址没有任何要求
  ) :
